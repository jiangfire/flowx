...
Tests use SQLite in-memory. Transaction-capable tests use `file:mem_N?mode=memory&cache=shared` (with atomic counter for uniqueness). Simple tests use `:memory:`.
